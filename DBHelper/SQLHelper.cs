...
namespace DBHelper
{
    ...
    public sealed partial class SQLHelper
    {
        ...
        /// <summary>
        ///默认初始化SqlHelper实例,读取配置文件中配置项SqlConnString
        /// </summary>
        public SQLHelper()
        {
            //this.ConnectionString = "" + ConfigurationManager.ConnectionStrings["SqlConnString"];
        }
        ...
        public int ExecuteNonQuery(string commandText, List<SqlParameter> parms)
        {
            return ExecuteNonQuery(commandText, parms.ToArray());
        }

        /// <summary>
        /// 执行SQL语句列表,返回影响的总行数
        /// </summary>
        /// <param name="commandList">SQL语句列表</param>
        /// <param name="parms">查询参数列表</param>
        /// <returns>返回影响的行数</returns>
        public int ExecuteNonQuery(List<string> commandList, List<SqlParameter[]> parmsList)
        {
            return ExecuteNonQuery(ConnectionString, CommandType.Text, commandList, parmsList);
        }
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回影响的行数</returns>
        public int ExecuteNonQueryProcedure(string commandText, params SqlParameter[] parms)
        {
            return ExecuteNonQuery(ConnectionString, CommandType.StoredProcedure, commandText, parms);
        }
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回结果集中的第一行第一列</returns>
        public object ExecuteScalarProcedure(string commandText, params SqlParameter[] parms)
        {
            return ExecuteScalar(ConnectionString, CommandType.StoredProcedure, commandText, parms);
        }
        ...
        /// <summary>
        /// 执行存储过程,返回结果集中的第一个数据表
        /// </summary>
        /// <param name="commandText">存储过程名称</param>
        /// <param name="parms">查询参数</param>
        /// <returns>返回结果集中的第一个数据表</returns>
        public DataTable ExecuteDataTableProcedure(string commandText, params SqlParameter[] parms)
        {
            return ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, commandText, parms).Tables[0];
        }
        ...
        {
            return ExecutePageDataTable(ConnectionString, sql, order, pageSize, pageIndex, parms, query, cte);
        }
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回结果集</returns>
        public DataSet ExecuteDataSetProcedure(string commandText, params SqlParameter[] parms)
        {
            return ExecuteDataSet(ConnectionString, CommandType.StoredProcedure, commandText, parms);
        }
        ...
        private static void PrepareCommand(SqlCommand command, SqlConnection connection
            ...
            , string commandText, SqlParameter[] parms)
        {
        ...
        ///  Created By Ny6000
        ///  Created Date:2017/1/25 15:58:25
        public static int ExecuteNonQuery(string connectionString, CommandType commandType
            , List<string> commandList, List<SqlParameter[]> parmsList)
        {
            int resultRows = 0;
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                for (int i = 0, j = commandList.Count; i < j; i++)
                {
                    resultRows += ExecuteNonQuery(connection, commandType, commandList[i], parmsList[i]);
                }
            }
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回结果集中的第一个数据表</returns>
        public static DataTable ExecuteDataTable(SqlConnection connection, CommandType commandType
            , string commandText, params SqlParameter[] parms)
        {
        ...
        /// <returns></returns>
        public static DataTable ExecutePageDataTable(string connectionString, string sql, string order
        ...
        #endregion 批量操作
        public bool ExecuteNonQuerySqlTrans(string cmdText, IList<IList<SqlParameter>> paramList)
        {
            bool bOK = false;
            ...
            try
            {
                ...
                for (int i = 0; i < paramList.Count; i++)
                {
                    ...
                    if (paramList != null && paramList[i] != null)
                    {
                        cmd = AddCommandParam(cmd, paramList[i]);
                    }
                    cmd.ExecuteNonQuery();
        ...
        protected static SqlCommand AddCommandParam(SqlCommand com, IList<SqlParameter> paramList)
        {
            if (com == null)
                return com;
